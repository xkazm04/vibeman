...
- ~~**Wave 7 — polish:**~~ **DONE** (all 7): system-map node spread widened (helpers + XRaySystemMap), PreviewModal exit animation restored, Manager Map empty-state copy, Tinder swipe-revert robust (identity-check + clamp, 7 variants), health-tooltip scroll reposition, Hot-Paths NaN guard, dead ComponentGrid deleted. NEW: `PreviewModal.tsx` has a pre-existing React-Compiler error on its `copyCode` useCallback (unrelated to edits). **Accurate ledger: 36 fixed (7 of 9 Criticals — remote #1/#2 deferred), 7 clean+fixable still open (Wave 8: ideas #1, workspace #2/#3, context #4, reflector #4/#5, scan-queue #5), 5 WIP-blocked, 2 remote-deferred.** The per-wave running reports overstated "all Criticals/all non-WIP closed" — corrected in `FIXES-SUMMARY.md`. Full run summary in `bug-ux-scan-2026-06-14/` (INDEX + FIXES-SUMMARY + FIXES-WAVE-1..7).
