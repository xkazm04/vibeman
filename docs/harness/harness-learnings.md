...
- ~~**Wave 8 — the missed clean findings:**~~ **DONE** (all 7): observability ingest validation + batch cap, git-branches concurrency bound + execFile, lifecycle orchestrator project-scoping, context drag-flush sync, file-watch notification real FK id, reflector cooldown → HTTP 429, reflector trigger loading-state wired. **Accurate final ledger: 43 fixed (7 of 9 Criticals — remote #1/#2 deferred by decision), 5 WIP-blocked (context #3, manager #1, taskrunner #2/#3/#4), 2 remote-deferred.** ALL clean findings closed. Full run summary in `bug-ux-scan-2026-06-14/` (INDEX + FIXES-SUMMARY + FIXES-WAVE-1..8).
