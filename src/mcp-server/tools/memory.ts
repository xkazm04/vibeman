...
export function registerMemoryTool(
  ...
  server.registerTool(
    'get_memory',
    {
      ...
      description:
        'Query the collective memory system for relevant knowledge about files, patterns, or problems. ' +
      ...
      inputSchema: z.object({
        query: z
          .string()
          .describe(
            'What you want to know about. Can be a file path, error message, pattern name, or description of the problem.'
          ),
        filePatterns: z
          .string()
          .optional()
          .describe(
            'JSON array of file patterns to match against (e.g., \'["src/app/features/TaskRunner","*.tsx"]\')'
          ),
        limit: z
          .number()
          .optional()
    ...
    async ({ query, filePatterns, limit }) => {
      ...
      const result = await client.get<MemoryResponse>(
      ...
      if (warnings.length > 0) {
        const items = warnings
          .map(
            (m) =>
              `- [${Math.round(m.effectiveness_score * 100)}% effective] **${m.title}**: ${m.description.slice(0, 200)}`
          )
          .join('\n');
