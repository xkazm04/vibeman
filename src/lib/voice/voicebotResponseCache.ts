...
export async function getCachedResponse(
  ...
  const result = await executeIndexedDBRead<CachedResponse | undefined>(
    openDatabase,
    STORE_NAME,
    (store) => store.get(key),
    { dbName: DB_NAME, storeName: STORE_NAME, operation: 'getCachedResponse' }
  );
...
export async function setCachedResponse(
  ...
  const result = await executeIndexedDBWrite<IDBValidKey>(
    openDatabase,
    STORE_NAME,
    (store) => store.put(entry),
    { dbName: DB_NAME, storeName: STORE_NAME, operation: 'setCachedResponse', onError: recoveryHandler }
  );
...
export async function deleteCachedResponse(key: string): Promise<void> {
  ...
  const result = await executeIndexedDBWrite<undefined>(
    ...
    { dbName: DB_NAME, storeName: STORE_NAME, operation: 'deleteCachedResponse' }
  );
...
export async function clearResponseCache(): Promise<void> {
  ...
  const result = await executeIndexedDBWrite<undefined>(
    ...
    STORE_NAME,
    (store) => store.clear(),
    { dbName: DB_NAME, storeName: STORE_NAME, operation: 'clearResponseCache' }
  );
...
export async function getResponseCacheStats(): Promise<{
  ...
  const result = await executeIndexedDBRead<CachedResponse[]>(
    openDatabase,
    STORE_NAME,
    (store) => store.getAll(),
    { dbName: DB_NAME, storeName: STORE_NAME, operation: 'getResponseCacheStats' }
  );
...
async function evictOldestEntries(maxEntries: number): Promise<void> {
  ...
  const result = await executeIndexedDBTransaction<void>(
    ...
    'readwrite',
    async (store) => {
      const entries = await promisifyRequest(store.getAll()) as CachedResponse[];
    ...
    { dbName: DB_NAME, storeName: STORE_NAME, operation: 'evictOldestEntries' }
  );
