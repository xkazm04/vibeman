...
function buildDesignPrompt(
  ...
  parts.push('15. **Event bus & inter-persona actions**: If the user describes coordination between multiple agents, cross-agent triggering, webhook reactions, or event-driven workflows, add the relevant instructions to the prompt. The persona system has a built-in event bus. Personas can trigger each other using `{"persona_action": {"target": "persona_name", "action": "execute|task", "input": {...}}}` and emit custom events using `{"emit_event": {"type": "...", "data": {...}}}`. When designing multi-agent workflows, include clear instructions about WHEN and WHY to trigger other personas, and what data to pass. For webhook-triggered personas, add instructions about expected webhook payload format.');
  const nextNum = 16;
