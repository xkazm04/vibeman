/**
 * Screen Coverage Prompt Template
 ...
 */
export function buildScreenCoveragePrompt(params: ScreenCoveragePromptParams): string {
  const {
    projectId,
    projectName,
    projectPath,
  ...
  const contextSlug = contextName.toLowerCase().replace(/\s+/g, '-');
...
2. **Buttons that open the feature**:
   ...
   - Look for \`data-testid\` attributes like:
     - \`data-testid="${contextSlug}-btn"\`
     - \`data-testid="open-${contextSlug}"\`
