...
/**
 * Build prompt for goal breakdown task
 */
...
The purpose is to break down this goal into actionable sub-tasks.
...
Break this goal down into actionable tasks:
...
### 3. Output Format
Please provide a structured breakdown of the goal.
`;
}
...
Purpose: Verify implementation progress for the goal.
...
- Look for code changes related to this goal
...
### 3. Goal Status Update
If the goal is complete, update the goal status:
\`\`\`bash
...
export async function createTask(request: TaskCreationRequest): Promise<CreatedTask | null> {
  try {
    ...
    switch (type) {
      ...
      case 'progress_check': {
        prompt = buildProgressCheckPrompt(goalId, goalTitle, contextName);
        break;
      }
