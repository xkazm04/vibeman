...
/**
 * Execute photo scan using Web Worker (offloads to background thread)
 */
export async function executePhotoScan(contextId: string): Promise<ScanResult> {
  ...
  try {
    ...
    const { executePhotoScanWithWorker } = await import('../blueprintScansWithWorkers');

    // Execute scan in Web Worker
    return await executePhotoScanWithWorker(contextId);
  } catch (error) {
