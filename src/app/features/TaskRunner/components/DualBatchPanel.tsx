...
import type { BatchState, BatchId } from '../lib/batchStorage';
...
export default function DualBatchPanel({
  ...
  // Helper to get queue items for a specific batch
  ...
  const renderBatch = (
    ...
    const queueItems = getBatchQueueItems(batch);
    ...
    const progress = batch.requirementIds.length > 0
      ? (batch.completedCount / batch.requirementIds.length) * 100
      : 0;
      ...
      >
        <div className={`relative bg-gray-800/40 border rounded-lg overflow-hidden ${
          ...
          <div className="relative p-3 flex items-center gap-4">
            {/* Left Side: Batch Info and Controls (350px) */}
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between text-[10px]">
                  <span className={isRunning ? 'text-blue-400 font-medium' : 'text-gray-500'}>
                    {batch.completedCount} / {batch.requirementIds.length}
                  </span>
  ...
  return (
    <div className="space-y-3 w-full">
      ...
      {renderBatch(batch1, 'batch1', 'Batch 1')}

      {/* Batch 2 - Show when conditions met */}
      {showBatch2 && renderBatch(batch2, 'batch2', 'Batch 2')}

      {/* Batch 3 - Show when conditions met */}
      {showBatch3 && renderBatch(batch3 ?? null, 'batch3', 'Batch 3')}

      {/* Batch 4 - Show when conditions met */}
      {showBatch4 && renderBatch(batch4 ?? null, 'batch4', 'Batch 4')}
    </div>
  );
}
