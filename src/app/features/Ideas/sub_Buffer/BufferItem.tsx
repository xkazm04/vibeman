        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          {(dependencyCount ?? 0) > 0 && (
            <div className="flex items-center gap-0.5" title={`${dependencyCount} linked ideas`}>
              ...
              <span className="text-2xs text-cyan-400 font-medium">{dependencyCount}</span>
            </div>
          )}
          ...
          {idea.provider && (
            <span
              className="text-micro px-1 py-0.5 rounded bg-gray-700/60 text-gray-500 font-medium uppercase leading-none"
              title={`Generated by ${idea.provider}${idea.model ? ` (${idea.model})` : ''}`}
            >
              {idea.provider.slice(0, 3)}
            </span>
          )}
          ...
          {idea.detailed === 1 && (
            <span
              className="text-micro px-1 py-0.5 rounded bg-amber-500/20 text-amber-400 font-medium leading-none"
              title="Implementation-ready with procedure steps"
            >
              DTL
            </span>
          )}
