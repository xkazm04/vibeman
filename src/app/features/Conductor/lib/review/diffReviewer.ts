 ...
 *
 * Uses the project's LLM endpoint at /api/llm/generate.
...
export async function reviewFileDiffs(
  ...
  const model = reviewModel || undefined;
...
Respond with ONLY valid JSON in this exact shape:
{
  ...
  if (reviewableFiles.length > 0) {
    try {
      ...
      const responseText = llmData.success ? llmData.response : '';
