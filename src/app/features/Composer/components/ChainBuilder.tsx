...
import {
  ...
  ChevronRight, Settings, X, Sparkles, ChevronDown, ChevronUp
} from 'lucide-react';
...
import PostChainEventEditor from './PostChainEventEditor';
...
export default function ChainBuilder({ onRun }: ChainBuilderProps) {
  ...
  const [showConditionalsConfig, setShowConditionalsConfig] = useState(false);
  const [showPostEventConfig, setShowPostEventConfig] = useState(false);
  ...
  // Handle adding a conditional branch
  const handleAddConditionalBranch = () => {
    ...
    const newBranch = {
      ...
      name: 'New Branch',
      condition: {
        type: 'event_check' as const,
        eventType: '',
        checkType: 'exists' as const,
      },
      trueChain: [],
      falseChain: [],
    };
    addConditionalBranch(selectedChainId, newBranch);
  };
              ...
              <div className="border-b border-gray-800/50 bg-gray-900/30">
                <div className="p-3 space-y-2">
                  ...
                  <div className="border border-gray-700/50 rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-xs font-medium text-gray-300">Trigger Configuration</span>
                      {showTriggerConfig ? (
                      ...
                      {showTriggerConfig && (
                        ...
                        >
                          <div className="p-3 bg-gray-900/20">
                            <TriggerSelector
                              trigger={selectedChain.trigger}
                              onChange={(trigger) => updateChainTrigger(selectedChain.id, trigger)}
                            />
                          </div>
                        </motion.div>
                      )}
                  ...
                  <div className="border border-gray-700/50 rounded-lg overflow-hidden">
                    <button
                      onClick={() => setShowConditionalsConfig(!showConditionalsConfig)}
                      className="w-full flex items-center justify-between px-3 py-2 bg-gray-800/30 hover:bg-gray-800/50 transition-colors"
                    >
                      <span className="text-xs font-medium text-gray-300">
                        Conditional Branches
                        {selectedChain.conditionalBranches && selectedChain.conditionalBranches.length > 0 && (
                          <span className="ml-2 text-[10px] text-violet-400">
                            ({selectedChain.conditionalBranches.length})
                          </span>
                        )}
                      </span>
                      {showConditionalsConfig ? (
                        <ChevronUp className="w-4 h-4 text-gray-500" />
                      ) : (
                        <ChevronDown className="w-4 h-4 text-gray-500" />
                      )}
                        ...
                        >
                          <div className="p-3 bg-gray-900/20">
                            <ConditionalBranchEditor
                              branches={selectedChain.conditionalBranches || []}
                              availableBlueprints={savedBlueprints}
                              onAddBranch={handleAddConditionalBranch}
                              onRemoveBranch={(branchId) => removeConditionalBranch(selectedChain.id, branchId)}
                              onUpdateBranch={(branchId, updates) =>
                                updateConditionalBranch(selectedChain.id, branchId, updates)
                              }
                            />
                          </div>
                        </motion.div>
                  ...
                  <div className="border border-gray-700/50 rounded-lg overflow-hidden">
                    <button
                      onClick={() => setShowPostEventConfig(!showPostEventConfig)}
                      className="w-full flex items-center justify-between px-3 py-2 bg-gray-800/30 hover:bg-gray-800/50 transition-colors"
                    >
                      <span className="text-xs font-medium text-gray-300">
                        Post-Chain Event
                        {selectedChain.postChainEvent?.enabled && (
                          <span className="ml-2 text-[10px] text-emerald-400">Enabled</span>
                        )}
                      </span>
                      {showPostEventConfig ? (
                        <ChevronUp className="w-4 h-4 text-gray-500" />
                    ...
                    <AnimatePresence>
                      {showPostEventConfig && (
                        ...
                        >
                          <div className="p-3 bg-gray-900/20">
                            <PostChainEventEditor
                              event={selectedChain.postChainEvent || null}
                              onChange={(event) => updatePostChainEvent(selectedChain.id, event)}
                            />
                          </div>
                        </motion.div>
                      )}
