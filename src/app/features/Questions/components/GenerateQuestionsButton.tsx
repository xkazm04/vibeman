...
interface GenerateQuestionsButtonProps {
  onGenerate: (questionsPerContext: number) => Promise<GenerateResult | void>;
...
export default function GenerateQuestionsButton({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await onGenerate(questionsPerContext);
      setStatus('success');
  ...
  const expectedQuestions = selectedCount * questionsPerContext;

  return (
    <div className="bg-gray-800/40 backdrop-blur-sm rounded-xl p-4 border border-gray-700/40">
      <div className="flex items-center justify-between gap-4">
        {/* Questions per context input */}
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-400">Questions per context:</label>
          <input
            type="number"
            min={1}
            max={10}
            value={questionsPerContext}
            onChange={(e) => setQuestionsPerContext(Math.max(1, Math.min(10, parseInt(e.target.value) || 3)))}
            className="
      ...
      {selectedCount > 0 && !message && (
        <p className="text-xs text-gray-500 mt-2">
          This will generate ~{expectedQuestions} questions total
        </p>
      )}
