...
/**
 * PackageStep - Fifth step (5/7) of the RefactorWizard workflow.
 *
 * This component handles AI-powered package generation and selection:
 * - If no packages exist yet, shows the package generation interface
 * - If packages exist, shows the package selection interface
 * - Allows filtering by impact level and bulk selection actions
 *
 * @component
 *
 * @example
 ...
 * - `packageFilter: PackageFilter` - Filter state { impact, effort, category, status }
 * - `packageGenerationStatus: 'idle' | 'generating' | 'completed' | 'error'` - Generation status
 * - `packageGenerationError: string | null` - Error message if generation failed
 * - `llmProvider: string` - Selected LLM provider for generation
 * - `llmModel: string` - Selected LLM model
 * - `selectedFolders: string[]` - Folder scope (for context display)
 * - `selectedOpportunities: Set<string>` - Selected opportunities to package
 * - `opportunities: RefactorOpportunity[]` - All opportunities (for filtering)
 *
 * **Actions Triggered:**
 * - `generatePackages()` - Trigger AI package generation
 * - `togglePackageSelection(packageId)` - Toggle individual package selection
 * - `selectAllPackages()` - Select all packages
 * - `clearPackageSelection()` - Deselect all packages
 * - `selectFoundationalPackages()` - Select packages with no dependencies
 * - `setPackageFilter(filter)` - Update filter state
 * - `setLLMProvider(provider)` - Update LLM provider
 * - `setLLMModel(model)` - Update LLM model
 * - `setCurrentStep('review')` - Navigate back to ReviewStep
 * - `setCurrentStep('execute')` - Navigate forward to ExecuteStep
 *
 * ## Data Flow
 *
 * **Input:**
 * - `selectedOpportunities` from refactorStore (opportunities to package)
 * - `llmProvider` and `llmModel` for AI configuration
 *
 * **Output (on generatePackages):**
 * - Calls `/api/refactor/generate-packages` POST with opportunities
 * - Updates `packages`, `projectContext`, `packageDependencies` in store
 * - Auto-selects foundational packages on completion
 *
 * ## Conditional Rendering
 * - **No packages (packages.length === 0)**: Shows generation view with ProviderSelector
 * - **Has packages**: Shows selection view with filter tabs and package list
 *
 * ## Package Generation API
 * ```
 * POST /api/refactor/generate-packages
 * Body: { opportunities, projectPath, selectedFolders, userPreferences: { provider, model } }
 * Response: { packages, context, dependencyGraph }
 * ```
 *
 * ## UI Features
 * - ProviderSelector for choosing AI provider
 * - Context summary showing folder scope and opportunity count
 * - Impact filter tabs (All, High Impact, Critical, Medium)
 * - Quick actions: Select All, Clear Selection, Select Foundational
 * - Animated package cards with metrics (issues, dependencies, effort)
 * - Foundational/Critical badges on packages
 */
