...
import { Map, Check, X, Loader2, Play, Square, AlertTriangle } from 'lucide-react';
        ...
        <div className="flex-shrink-0 px-3 py-1.5 border-b border-cyan-500/5 overflow-x-auto custom-scrollbar">
          <div className="flex items-center gap-2 min-w-max">
            {batchProjects.map((p, i) => (
              <div
                key={p.id}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-2xs transition-all ${
              ...
              >
                ...
                {p.status === 'pending' && <span className="w-2.5 h-2.5 text-zinc-400 flex items-center justify-center">○</span>}
                <span
                  className={
                    p.status === 'running'
