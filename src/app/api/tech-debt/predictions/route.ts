/**
 * Debt Predictions API (Consolidated)
...
} from '@/app/features/TechDebt/lib/predictionEngine';
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('[TechDebt Predictions API] GET error:', { data: error });
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (files && Array.isArray(files)) {
      ...
      for (const filePath of filesToAnalyze) {
        try {
          ...
          logger.warn(`[TechDebt Predictions] Failed to read file: ${filePath}`);
        }
      }
    }
    ...
    logger.error('[TechDebt Predictions API] POST error:', { data: error });
    return NextResponse.json(
...
async function scanProjectFiles(projectPath: string): Promise<string[]> {
  ...
  function walkDir(dir: string) {
    try {
      ...
      logger.warn(`[TechDebt Predictions] Failed to read directory: ${dir}`);
    }
  }
